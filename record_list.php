                            ...
                            <td class="text-center">
                                <div class="d-flex w-100 justify-content-center align-items-center">
                                    ...
                                    <div class="col-auto px-1">
                                        <a href="./med_cert_pdfs/<?= $row['code'] ?>.pdf" target="_blank"
                                            download="<?= $row['code'] ?>.pdf" class="text-info"
                                            title="Download Med Cert PDF File"><i
                                                class="material-icons opacity-6 me-2 text-md">download</i></a>
                                    </div>
                                    <div class="col-auto px-2">
                                        <a href="./med_cert_pdfs/<?= $row['code'] ?>.pdf" target="_blank"
                                            class="text-black" title="View Med Cert"><i
                                                class="material-icons opacity-6 me-2 text-md">visibility</i></a>
                                    </div>
                                    <?php endif; ?>

                                    <div class="col-auto px-2">
                                        <a href="./?page=manage_record&id=<?= $row['id'] ?>" class="text-dark"
                                            title="Edit Med Cert Details"><i
                                                class="material-icons opacity-6 me-2 text-md">edit</i></a>
                                    </div>
                                    <div class="col-auto px-2">
                                        <a href="javascript:void(0)" data-id="<?= $row['id'] ?>"
                                            data-code="<?= $row['code'] ?>" class="text-primary delete_record"
                                            title="Edit Med Cert Details"><i
                                                class="material-icons opacity-6 me-2 text-md">delete</i></a>
                                    </div>
                                </div>
                            </td>
                        </tr>
        ...
        if (confirm("Are you sure to delete '" + code + "'?") === true) {
            ...
            $.ajax({
                ...
                error: err => {
                    ...
                    el.show('slow')
                    $('html, body').scrollTop($('#record_list').parent().offset().top -
                        '150')
                    end_loader()
                },
